...
@click.command(
    help="Train a minimal Transformer NER baseline.",
    context_settings={"show_default": True},
)
@click.option("--train", type=click.Path(path_type=Path), default=Path("train.jsonl"))
@click.option("--dev", type=click.Path(path_type=Path), default=Path("dev.jsonl"))
@click.option("--output-dir", type=click.Path(path_type=Path), default=Path("artifacts/baseline"))
@click.option("--model-name", default=DEFAULT_MODEL)
...
@click.option("--stride", type=int, default=DEFAULT_STRIDE)
@click.option("--seed", type=int, default=42)
@click.option("--device", type=click.Choice(("auto", "cpu", "cuda")), default="auto")
@click.option("--num-workers", type=int, default=0)
@click.option("--max-train-records", type=int)
@click.option("--max-dev-records", type=int)
@click.option("--overwrite-output-dir", is_flag=True)
