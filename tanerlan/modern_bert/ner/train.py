...
from tanerlan.modern_bert.ner.config import TrainingConfig
from tanerlan.modern_bert.ner.data_module import NerDataModule
from tanerlan.modern_bert.ner.training_module import (
    NerTrainingModule,
...
def setup_strategy(
    ...
    parallel_devices: list[torch.device] = accelerator_class.get_parallel_devices(
        num_devices if accelerator_class is CPUAccelerator else device_ids
    )
    ...
    match strategy_settings:
        ...
        case FSDP1StrategyConfig():
            if num_devices < 2:
                raise ValueError("FSDP strategy requires at least two devices.")
            strategy = FSDPStrategy(
                accelerator=accelerator,
                parallel_devices=parallel_devices,
                param_dtype=strategy_settings.param_dtype,
                reduce_dtype=strategy_settings.reduce_dtype,
                buffer_dtype=strategy_settings.buffer_dtype,
            )
...
def _dump_run_metadata(
    work_dir: Path, config: TrainingConfig, datamodule: NerDataModule
) -> None:
    """Конфиг эксперимента, схема меток и статистика данных рядом с чекпоинтами."""
    ...
    (work_dir / "labels.json").write_text(
        json.dumps(
            {
                "entity_types": datamodule.schema.entity_types,
                "tags": datamodule.schema.tags,
                "tag2id": datamodule.schema.tag2id,
            },
            ensure_ascii=False,
            indent=2,
        ),
    ...
    (work_dir / "dataset_stats.json").write_text(
        json.dumps(datamodule.stats, ensure_ascii=False, indent=2), encoding="utf-8"
    )
...
def run(config_path: Path, experiment_registry: Path) -> None:
    ...
    ### DataModule setup: схема меток нужна модели до fit ###
    ...
    _dump_run_metadata(WORK_DIR, config, datamodule)

    ### TrainingModule setup ###
    training_module = create_training_module(config=config, schema=datamodule.schema)

    ### Strategy, Callbacks and logger setup ###
    logger = TensorBoardLogger(
        save_dir=WORK_DIR, name="tb_logs", default_hp_metric=False
    )
    logger.log_hyperparams(
        {
            **config.model_dump(mode="json"),
            "labels": {"tags": datamodule.schema.tags},
            "dataset_stats": datamodule.stats,
        }
    )
    ...
    lightning_module = cast(NerTrainingModule, trainer.strategy.lightning_module)
    ...
    if ckpt_callback is not None:
        if ckpt_callback.best_model_path:
            ...
            module_logger.warning(
                "No checkpoint path found, but ModelCheckpoint is provided! Saving the in-memory model as HF model."
            )
    ...
    if ckpt_path is not None:
        ...
        module_logger.info(
            f"Loaded {ckpt_type} checkpoint from {ckpt_path} "
            f"({config.checkpointing.monitor}={ckpt_callback.best_model_score if ckpt_callback else None})."
        )
